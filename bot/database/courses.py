...
async def get_all_courses(active_only: bool = False) -> list[dict]:
    ...
    cursor = db_module.db.courses.find(query)
    return await cursor.to_list(None)

async def get_course(course_id: str) -> dict | None:
    return await db_module.db.courses.find_one({"_id": ObjectId(course_id)})

async def get_course_by_channel(channel_id: int) -> dict | None:
    return await db_module.db.courses.find_one({"channel_id": channel_id})
...
async def create_course(
    ...
    result = await db_module.db.courses.insert_one({
        "name": name,
...
async def update_course(course_id: str, data: dict):
    await db_module.db.courses.update_one(
